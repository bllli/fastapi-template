...
class Service:
    ...
    async def process(self):
        await self.client.set("test", "1")
        got = await self.client.get("test")
        return got
...
class DI(containers.DeclarativeContainer):
    ...
    # service = providers.Singleton(Service, client=client)  # 错误使用时，依然可以恢复
...
async def index(service: Service = Depends(Provide[DI.service])):
    print("service", id(service), id(service.client))
